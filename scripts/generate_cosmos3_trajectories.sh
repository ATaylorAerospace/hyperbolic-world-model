#!/usr/bin/env bash
# Generate action-conditioned video rollouts with Cosmos 3 Nano (forward_dynamics), then extract tokenizer latents.
# Cosmos is used in inference mode only (docs/cosmos3_usage.md). Requires NVIDIA's cosmos-framework,
# ffmpeg and a GPU on this machine; nothing in this repository installs them.
#
