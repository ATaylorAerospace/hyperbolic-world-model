...
uv run pytest                 # 468 tests, CPU, < 60 s
...
6. **Contributions are licensed to the project.** By opening a pull request you confirm that you
   wrote the contribution (or have the right to submit it) and you grant the copyright holder a
   perpetual, worldwide, irrevocable licence to use, modify, sublicense and relicense it, including
   under commercial terms, so the project can change its licence in future without tracking down
   every contributor. The project is currently under PolyForm Noncommercial 1.0.0.
7. **No fabricated outputs.** The README's Verification section is the summary of a real `pytest -q`
   run (per-suite counts from `pytest --collect-only`); tables in `outputs/report/` are generated by
   `scripts/make_report.sh`. Do not hand-edit either.
